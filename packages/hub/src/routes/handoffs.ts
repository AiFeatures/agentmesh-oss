...
export const handoffRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get<{ Params: { workspace: string } }>(
    "/api/v1/workspaces/:workspace/handoffs/retry-count-distribution",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const rows = db
        .prepare(
  ...
  app.get<{ Params: { workspace: string } }>(
    "/api/v1/workspaces/:workspace/handoffs/handoff-notes-count",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const rows = db
        .prepare(
          `SELECT hn.handoff_id, COUNT(*) AS note_count
           FROM handoff_notes hn
           JOIN handoffs h ON h.handoff_id = hn.handoff_id
           WHERE h.workspace_id = ?
           GROUP BY hn.handoff_id
           ORDER BY note_count DESC
           LIMIT 20`,
        )
        .all(req.params.workspace) as {
  ...
  app.get<{ Params: { workspace: string } }>(
    "/api/v1/workspaces/:workspace/handoffs/handoff-context-size",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const row = db
        .prepare(
          `SELECT COUNT(*) AS total,
                  AVG(LENGTH(COALESCE(context, ''))) AS avg_size,
                  MAX(LENGTH(COALESCE(context, ''))) AS max_size,
                  MIN(LENGTH(COALESCE(context, ''))) AS min_size
           FROM handoffs
           WHERE workspace_id = ?`,
        )
        .get(req.params.workspace) as {
  ...
  app.get<{ Params: { workspace: string } }>(
    "/api/v1/workspaces/:workspace/handoffs/handoff-context-size-avg",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const row = db
        .prepare(
          `SELECT COUNT(*) AS total,
                  ROUND(AVG(LENGTH(context)), 1) AS avg_context_length,
                  MAX(LENGTH(context)) AS max_context_length,
                  MIN(LENGTH(context)) AS min_context_length
           FROM handoffs
           WHERE workspace_id = ? AND context IS NOT NULL`,
        )
        .get(req.params.workspace) as {
  ...
  app.get<{ Params: { workspace: string } }>(
    "/api/v1/workspaces/:workspace/handoffs/handoff-timeout-utilization",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const rows = db
        .prepare(
          `SELECT handoff_id, summary, timeout_seconds,
                  CASE WHEN expires_at IS NOT NULL AND updated_at IS NOT NULL
                    THEN ROUND(
                      (julianday(updated_at) - julianday(created_at)) * 86400
                      / NULLIF(timeout_seconds, 0) * 100, 1
                    )
                    ELSE NULL
                  END AS utilization_pct
           FROM handoffs
           WHERE workspace_id = ? AND timeout_seconds > 0 AND status = 'completed'
           ORDER BY utilization_pct DESC
           LIMIT 20`,
        )
        .all(req.params.workspace) as {
  ...
  // F-523 handoff-status-daily
  app.get<{ Params: { workspaceId: string } }>(
    "/api/v1/workspaces/:workspaceId/analytics/handoff-status-daily",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const { workspaceId } = req.params;
      const rows = db
  ...
  // F-526 handoff-avg-context-size
  app.get<{ Params: { workspaceId: string } }>(
    "/api/v1/workspaces/:workspaceId/analytics/handoff-avg-context-size",
    { preHandler: app.authGuard },
    async (req, reply) => {
      const { workspaceId } = req.params;
      const row = db
      ...
      return reply.send({
        total: row?.total ?? 0,
        avg_size: row?.avg_size ? +Number(row.avg_size).toFixed(2) : 0,
        max_size: row?.max_size ?? 0,
        min_size: row?.min_size ?? 0,
      });
    },
  );
  ...
  app.get(
    "/api/v1/workspaces/:workspace/handoffs/handoff-rejection-reason-frequency",
    { preHandler: app.authGuard },
    async (request, reply) => {
      const { workspace } = request.params as { workspace: string };
      const rows = db
        .prepare(
          `SELECT to_agent_id, COUNT(*) as rejection_count
           FROM handoffs WHERE workspace_id = ? AND status = 'rejected'
           GROUP BY to_agent_id ORDER BY rejection_count DESC LIMIT 20`,
        )
        .all(workspace) as Array<{ to_agent_id: string; rejection_count: number }>;
